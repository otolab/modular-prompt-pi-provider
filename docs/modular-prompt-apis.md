...
| `maxTokens` | `maxTokens` | |
| `reasoning: "low"\|"medium"\|"high"` | `reasoningEffort` | MLX thinking 深度 |
...
### `ModelSpec.defaultOptions`（別レイヤ）

`ApplicationConfig.models[].defaultOptions` は `MlxDriver.defaultOptions` にマージされ **Python 層まで届く**。`maxTokens` / `temperature` / `topP` / `topK` のみ。`mode` は入れない（`src/driver/mlx-options.ts` の `pickMlxDriverDefaultOptions`）。
