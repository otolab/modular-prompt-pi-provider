...
| **P0** | Pi `Usage` マッピング | `result.usage` → `output.usage`（[adapter.md](./adapter.md)） |
| **P0** | `signal` 伝播と abort 変換 | `QueryOptions.signal` に渡し、`finishReason: 'error'` + `signal.aborted` → `"aborted"`（[abort-spec.md](./abort-spec.md)） |
...
| **本拡張** | `streamSimple`、型変換、生ストリーム → Pi イベント、`result` → Pi `Usage`、abort → Pi `stopReason`、compact Prompt | MLX 子プロセス制御、driver API の変更 |
...
- 固定モデル 1 つ、`streamSimple`、usage マッピング、signal 伝播
- `pi install` でロード確認
