...
新スキーマ（Issue #40）の詳細は [workflow.md](./workflow.md) を参照。
...
| 項目 | 挙動 |
|---|---|
| 複数モデル | YAML `models` マップ（またはレガシー配列）で宣言 |
| Pi `model.id` | **論理名**（物理パスは直接触らせない） |
| `defaultQueryOptions` | 各 model 定義で**必須** |
...
モデル選択・workflow 解決の詳細は [workflow.md](./workflow.md)（Issue #40）。
...
|---|---|---|
| `cacheDir` / `pythonPath` / `defaultQueryOptions` | `modular-prompt-provider/config.yaml` | **読み込み・適用** |
